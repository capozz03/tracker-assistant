...
Входящее сообщение
  ...
  ├── /project                  → список проектов Timetta API (постранично, 8 на страницу)
  ...
  ├── Нажатие inline-кнопки    → CallbackQueryHandler (выбор проекта / пагинация / подтверждение)
  ├── Текст (обычный)           → generate_tasks → превью + [✅ Создать] [❌ Отмена]
  ├── Текст (ждём подтверждения)→ трактуется как правка → перегенерация превью
  ├── Пересланное сообщение     → извлечение текста → превью
  ├── Фото + подпись            → generate_tasks(подпись) → превью (фото прикрепится после создания)
  ├── Фото без подписи          → attach_file к последним задачам сессии
  ├── Файл + подпись            → generate_tasks(подпись) → превью
  └── Файл без подписи          → attach_file к последним задачам сессии
```

> С версии «ревью задач» текст и медиа-с-подписью **не создают задачи сразу** —
> сначала показывается превью на проверку. Подробнее ниже.

---

## Ревью задач перед созданием

Чтобы исключить выгрузку в Timetta непроверенной постановки/декомпозиции, бот
сначала **формирует** задачи и показывает их на подтверждение, и только после
нажатия «Создать» выгружает в Timetta.

```
требования → generate_tasks (только Claude) → превью + [✅ Создать] [❌ Отмена]
  ├─ ✅ Создать  → create_tasks (выгрузка в Timetta) → ответ с URL (+ прикрепление фото, если было)
  ├─ ❌ Отмена   → pending очищается, «❌ Отменено»
  └─ текст       → правка → generate_tasks(требования + правка) → обновлённое превью
```

### Превью

На каждую задачу выводится:

```
1. <summary>
   <описание, обрезано до ~150 симв>
   👤 <исполнитель>   🏷 <теги через запятую>
```

Заголовок: `📋 Проверь задачи (N). Всё ок — нажми «Создать», или пришли правки текстом.`
Исполнитель и теги отображаются человекочитаемыми именами (UUID → `displayName` /
имя тега), при отсутствии — `—`.

### Кнопки подтверждения

| Кнопка | callback | Действие |
|---|---|---|
| ✅ Создать | `submit_ok:` | `create_tasks` → выгрузка в Timetta, ответ с URL, очистка pending |
| ❌ Отмена | `submit_cancel:` | очистка pending, сообщение «❌ Отменено» |

### Правки свободным текстом

Пока есть неподтверждённый набор (`pending_submit`), **любой текст трактуется как
правка**: он добавляется к исходным требованиям (`requirements + "\n\nПравки от
пользователя:\n" + текст`), набор перегенерируется, показывается новое превью.
Чтобы начать с нуля — нажмите **❌ Отмена**, затем отправьте новые требования.

### Фото / файл с подписью

Подпись уходит в `generate_tasks`, показывается превью. Сам файл **не скачивается
сразу** — в `pending_submit["media"]` хранится только `file_id`. После нажатия
«Создать» файл скачивается по `file_id` и прикрепляется к созданным задачам.
Фото/файл без подписи по-прежнему прикрепляется к последним задачам сессии.

> Состояние — лёгкий in-memory флаг `chat_data["pending_submit"]` (без
> `ConversationHandler`), сбрасывается при рестарте бота. Одно pending на чат;
> новая генерация перезаписывает предыдущую.
